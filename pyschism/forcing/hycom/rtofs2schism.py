            ...
            for boundary in gdf.itertuples():
                ...
                if elev2D:
                    ...
                    if cached:
                        ...
                        time_idx, lon_idx1, lon_idx2, lat_idx1, lat_idx2, x2, y2 = (
                            get_idxs(date, ds, bbox)
                        )
                        ssh = np.squeeze(
                            ds["ssh"][
                                time_idx + 1,
    ...
    def fetch_data(
        ...
        for it, date in enumerate(timevector):
            ...
            if cached:
                ...
                ds = Dataset(salt_url)
                time_idx, lon_idx1, lon_idx2, lat_idx1, lat_idx2, x2, y2 = get_idxs(
                    date, ds, bbox
                )
...
class DownloadRTOFS:
    ...
    def fetch_data(self, startdate, rnday=9):
        ...
        idx_y2 = 1634
        logger.info(
            f"idx_x1 is {idx_x1}, idx_x2 is {idx_x2}, idx_y1 is {idx_y1}, idx_y2 is {idx_y2}"
        )
        enddate = startdate + timedelta(days=rnday)
        datevector = np.arange(
            startdate, enddate, np.timedelta64(1, "D"), dtype="datetime64"
        )
        ...
        for it, date in enumerate(datevector):
            ...
            for ivar, (key, var) in enumerate(vars.items()):
                ...
                if key == "ssh":
                    var[1].append(
                        values[
                            it * 8, 0, idx_y1 : idx_y2 + 1, idx_x1 : idx_x2 + 1
                        ].astype("float32")
                    )
                else:
                    var[1].append(
                        values[it, :, idx_y1 : idx_y2 + 1, idx_x1 : idx_x2 + 1].astype(
                            "float32"
                        )
                    )
