...
def put_sflux_fields(
    iday, date, timevector, ds, nx_grid, ny_grid, air, rad, prc, output_interval, OUTDIR
):
    ...
    if air is True:
        with Dataset(
            OUTDIR / "sflux_air_1.{:04}.nc".format(iday + 1),
            "w",
            ...
            dst["time"].units = (
                f"days since {rt.year}-{rt.month}" f"-{rt.day} 00:00 UTC"
            )
    ...
    if prc is True:
        with Dataset(
            OUTDIR / "sflux_prc_1.{:04}.nc".format(iday + 1),
            "w",
            format="NETCDF3_CLASSIC",
        ) as dst:
            dst.setncatts({"Conventions": "CF-1.0"})
            ...
            dst["prate"][:, :, :] = ds["mtpr"][
                idx : idx + 25 : output_interval, ::-1, :
            ]

    if rad is True:
        with Dataset(
            OUTDIR / "sflux_rad_1.{:04}.nc".format(iday + 1),
            "w",
            format="NETCDF3_CLASSIC",
        ) as dst:
            dst.setncatts({"Conventions": "CF-1.0"})
...
class ERA5(SfluxDataset):
    ...
    def write(
        ...
        self.inventory = ERA5DataInventory(
            self.start_date, self.rnday, bbox, tmpdir=tmpdir
        )
