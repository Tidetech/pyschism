...
class AWSGrib2Inventory:
        ...
        for it, dt in enumerate(requested_dates[: n_cycles * 6 + 1]):
            ...
            while levels:
                ...
                fhour2 = (
                    fhour + i * 6
                    if cycle_index == 0
                    else fhour - cycle_index * 6 + i * 6
                )
                if self.product == "conus":
                    file_metadata.setdefault(dt, []).append(
                        f"hrrr.{date2}/{self.product}/hrrr.t{cycle}z.wrfsfcf{fhour2:02d}.grib2"
                    )
                elif self.product == "alaska":
                    file_metadata.setdefault(dt, []).append(
                        f"hrrr.{date2}/{self.product}/hrrr.t{cycle}z.wrfsfcf{fhour2:02d}.ak.grib2"
                    )
                levels -= 1
                i += 1
    ...
    def files(self):
        grbfiles = glob.glob(
            f'{self.tmpdir}/hrrr.{self.forecast_cycle.strftime("%Y%m%d")}/hrrr_wrfsfcf*.grib2'
        )
        grbfiles.sort()
        return grbfiles
...
class HRRR:
    ...
    def write(
        self, start_date, rnday, air: bool = True, prc: bool = True, rad: bool = True
    ):
        ...
        if (start_date + timedelta(days=rnday)) > datetime.utcnow():
            logger.info(
                f"End date is beyond current time, set rnday to one day and record to 2 days"
            )
            rnday = 1
            self.record = 2  # days
        ...
        npool = (
            len(datevector)
            if len(datevector) < mp.cpu_count() / 2
            else mp.cpu_count() / 2
        )
    ...
    def gen_sflux(
        ...
        if air:
            ds = xr.Dataset(
                ...
                coords={
                    "time": np.round(
                        (times - date.to_datetime64()) / np.timedelta64(1, "D"), 5
                    ).astype("float32"),
            ...
            # write to file
            ds.to_netcdf(
                self.outdir / f"sflux_air_{self.level}.{istack:04d}.nc",
                "w",
                "NETCDF3_CLASSIC",
                unlimited_dims="time",
            )
        ...
        if prc:
            ...
            ds.prate.attrs = {"units": "kg m-2 s-1", "long_name": "Precipitation rate"}

            # write to file
            ds.to_netcdf(
                self.outdir / f"sflux_prc_{self.level}.{istack:04d}.nc",
                "w",
                "NETCDF3_CLASSIC",
                unlimited_dims="time",
            )
        ...
        if rad:
            ...
            # write to file
            ds.to_netcdf(
                self.outdir / f"sflux_rad_{self.level}.{istack:04d}.nc",
                "w",
                "NETCDF3_CLASSIC",
                unlimited_dims="time",
            )
