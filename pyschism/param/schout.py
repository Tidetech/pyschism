...
class SurfaceOutputVars:
    ...
    def __init__(self):
        self._surface_output_vars = {
            "iof_hydro": [(var.value, i) for i, var in enumerate(IofHydroVariables)],
            "iof_dvd": [(var.value, i) for i, var in enumerate(IofDvdVariables)],
            "iof_wwm": [(var.value, i) for i, var in enumerate(IofWwmVariables)],
            "iof_gen": [(var.value, i) for i, var in enumerate(IofGenVariables)],
            "iof_age": [(var.value, i) for i, var in enumerate(IofAgeVariables)],
            "iof_sed": [(var.value, i) for i, var in enumerate(IofSedVariables)],
            "iof_eco": [(var.value, i) for i, var in enumerate(IofEcoVariables)],
            "iof_icm": [(var.value, i) for i, var in enumerate(IofIcmVariables)],
            "iof_cos": [(var.value, i) for i, var in enumerate(IofCosVariables)],
            "iof_fib": [(var.value, i) for i, var in enumerate(IofFibVariables)],
            "iof_sed2d": [(var.value, i) for i, var in enumerate(IofSed2dVariables)],
            "iof_marsh": [(var.value, i) for i, var in enumerate(IofMarshVariables)],
            "iof_ice": [(var.value, i) for i, var in enumerate(IofIceVariables)],
            "iof_ana": [(var.value, i) for i, var in enumerate(IofAnaVariables)],
        }
...
class OutputVariableDescriptor:
    ...
    def __set__(self, obj, val: bool):
        if not isinstance(val, bool):
            raise TypeError(
                f"Argument to {self._name} must be boolean, not " f"type {type(val)}."
            )
...
class SCHOUT(metaclass=SchoutMeta):
    ...
    def __init__(self, nhot_write: int = None, nspool_sta: int = None, **outputs):
        """
        nhot_write:
    ...
    def __str__(self):
        ...
        if self.nspool_sta is not None:
            ...
            if isinstance(nspool_sta, float):
                nspool_sta = int(round(timedelta(hours=nspool_sta) / self.dt))
